...
def loadcsvtoDf(filename, delimiter=";"):
    # try:
    dftime = pd.read_csv(filename, delimiter=delimiter, nrows=10000)
    """
    mylist = []
    for chunk in pd.read_csv(filename, delimiter=delimiter, low_memory=False, chunksize=2000):
        mylist.append(chunk)
    dftime = pd.concat(mylist, axis=0)
    """
